    ...
    <div class="right">
        ...
        <div class="cen_box">
            ...
            <div class="bor_box">
                ...
                <dl class="cf">
                    <dt><em class="asterisk">*</em>设置分类： </dt>
                    <select id="cateId" class="puiSelect" style="width: 184px;">
                        <option value="0">请选择</option>
                        <?php
                        foreach($tree as $value){
                            $selected = '';
                            if($value['id'] == $item->cateId){
                                $selected="selected";
                            }
                    ...
                    <div class=" member_cen">
                        <table class="thead tbody_cen">
                            ...
                            foreach ($itemAttr as $key => $value) {
                               ...
                               echo '<div id="uploadForm" style="display: inline;">';
                               echo '    <input id="file" type="file" style="width:200px;border:0;"/>';
                               echo '    <button id="upload" type="button" onclick="upload_img(this)" style="padding: 3px 13px;">上传</button>';
                               echo '</div> <input type="hidden" name="id[]" value="'.$value->id.'">';
                               echo '    </td>';
                               ...
                               echo '    <td><a href="javascript:;" onclick="del(this,'.$value->id.')">删除</a></td>';
                               echo '</tr>     ';
...
<script type="text/javascript">
    ...
    function save()
    {
        ...
        $('img[name=picsList]').each(function () {
            ...
            var srclist = src.indexOf('_')>0 ? src.split('_') : src;
            if(src.indexOf('_')>0){
                var srclist = src.split('_');
                var nsrc = 'upload' + srclist[0].split('upload')[1] + '.' + srclist[1].split('.')[1];
            }else{
                var nsrc = 'upload' + srclist.split('upload')[1];
            }
        ...
        $.post("<?php echo input::site('admin/item/update') ?>", {
            'itemId': itemId,
            'title': title,
            'picskey': picskey,
            'picsvalue': picsvalue,
    ...
    // 删除属性栏
    function del(tr,id){
        $.post('<?php echo input::site('admin/item/delAttr') ?>',{id:id},function(data){
            var da = eval('('+data+')');
            if(da.success == 0){
                alert(da.msg);
            }else{
