...
class comm_ext
{
                ...
                if($userName!='admin')
                {
                    ...
                    if($validId)
                    {
                        if (!self::hasPower($validId))
                        {
                            header("Content-type: text/html; charset=utf-8");
                            echo "  <script type=\"text/javascript\" chartset=\"utf-8\">
                                alert(\"对不起，你没有此模块的操作权限!\");history.go(-1);
                            </script>";
                        }
                    }
                }
    ...
    {
        if ($_SESSION['uid'] >1)//不记录admin的操作情况
        {
            $model		= M('log');
            return $model->insert(array('uid'=>$_SESSION['uid'],'tid'=>$tid,'msg'=>input::ipAddr().$msg,'ctime'=>time()));
        }
    }
    ...
    public static function getQrcodeUrl($itemId)
    {
        $cache = cache::getClass();
        $sign = md5('30'.time());
        $cache->setex('orderQrcode:'.$itemId,5,$sign);
        return input::site('wechat/order/couponQrcode?id='.$itemId.'&sign='.$sign);
    }
