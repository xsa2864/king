...
class Login_Controller extends Template_Controller 
{
    ...
    public function index(){
        ...
        if (isset($_SESSION['header_uid'])){
          echo "<script type=\"text/javascript\" chartset=\"utf-8\">location.href= '".input::site('headquarters/display')."';</script>";
        }else{
            $capt           = captcha::getClass();
            $data['capt']   = $capt;               
            $view = new View('headquarters/login_view', $data);
            $view->render();
        }
    }
    ...
    public function checkLogin(){
        ...
        if ($capt->valid(P('captcha'))){                              
            ...
            if (!$userinfo){
                ...
                $_SESSION['header_uid']    = $userinfo->id;
