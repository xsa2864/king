<?php defined('KING_PATH') or die('访问被拒绝.');
      class Item_Controller extends Template_Controller
      {
          ...
          public function qrcode(){
            $rs = M('set_config')->getOneData("account_id=".$this->account_id,'qr_code');
            if($rs->qr_code == 1){
              $qrcode = qrcode::getClass();
              $qrcode->png('http://www.hlcmj.com',false,0,15,4);              
            }else{
              echo "<meta charset=utf-8>二维码显示已关闭";
            }
          }
          /**
           * 在售商品
          ...
          {
              $where = 'i.account_id='.$this->account_id;
              $rs = M('set_config')->getOneData("account_id=".$this->account_id,'warning_stock');
              
              // 状态或库存条件
              if($status <= 1){
                $where .= " and i.status=$status";                
              }else if($status == 2){
                $where .= " and attr_stock=0";   
              }else if($status == 3){     
                $where .= " and attr_stock<=".$rs->warning_stock;               
              }

              if(G('keyword')&&G('keyword')!=''){
                  $where .= " and i.title like '%".G('keyword')."%'";   
                  $data['keyword'] = G('keyword');
              }
              if(G('itemId')&&G('itemId')!=''){
                  $where .= " and i.id=".G('itemId'); 
                  $data['itemId'] = G('itemId');
              }
              if(G('categroy1')&&G('categroy1')>0){
                  $where .= " and i.cateId=".G('categroy1'); 
                  $data['cateId'] = G('categroy1');
              }
            

              $sql = "SELECT i.id,i.pics,i.title,i.points attr_jifen,i.price attr_price,i.stock attr_stock FROM tf_item i LEFT JOIN tf_item_attr ia ON ia.item_id=i.id where ".$where." group by i.id ORDER BY addtime DESC";
              $total		            = M()->query($sql);
              $data['pagination']   = pagination::getClass(array(
                  'segment'           => 'page',
                  'total'             => $total,
              ...
              $hql = $sql." limit $start,20";
              $data['itemList'] = M()->query($hql);

              // 前端的显示效果  
              $data['sales']    = M('item')->getAllCount(array('status'=>1,'account_id'=>$this->account_id));
              $data['downNum']  = M('item')->getAllCount(array('status'=>0,'account_id'=>$this->account_id));
              //售完商品
              $where = 'i.account_id='.$this->account_id.' and attr_stock=0';
              $sql = "SELECT i.id,i.pics,i.title,i.points attr_jifen,i.price attr_price,i.stock attr_stock FROM tf_item i LEFT JOIN tf_item_attr ia ON ia.item_id=i.id where ".$where." group by i.id ORDER BY addtime DESC";
              $arr_sell = M()->query($sql);
              $data['sellOut']  = count($arr_sell);
              // 警戒库存商品
              $where = 'i.account_id='.$this->account_id.' and attr_stock<='.$rs->warning_stock;
              $sql = "SELECT i.id,i.pics,i.title,i.points attr_jifen,i.price attr_price,i.stock attr_stock FROM tf_item i LEFT JOIN tf_item_attr ia ON ia.item_id=i.id where ".$where." group by i.id ORDER BY addtime DESC";

              $warm_sell = M()->query($sql);
              $data['warm']     = count($warm_sell);
              $data['status'] = $status;
          ...
          public function saveprice(){
              ...
              $re_msg['success'] = 0;
              if($id>0){
                $data['attr_price'] = $price;
                $where['item_id'] = $id;
                $rs = M('item_attr')->update($data,$where);
                if($rs){
                  $re_msg['success'] = 1;
                }
              }
          ...
          {
              $id                       = S('edit');
              $data['item']             = M('item')->getOneData(array('id'=>$id,'account_id'=>$this->account_id));
              $time						          = time();
              $data['validKey']			    = md5($GLOBALS['config']['md5Key'].$time);
              $data['validTime']        = $time;
              $data['tree']             = str_ext::getTree();
          ...
          public function see()
          {
              ...
              $this->template->content  = new View('admin/item/see_view',$data);
              $pic = D('picture')->GetData();
              $this->template->tipBoxContent3 = new View('admin/picture/useImg_view', $pic);
              $this->template->render();
          }
          ...
          public function update(){   
              ...
              if($return){                  
                  ...
                  foreach($attr_name as $key=>$vala){
                     
                      if(!empty($vala)){
                           $allStock = $attr_stock[$key];
                           $att_data['attr_name']  = $vala;
                           $att_data['attr_price'] = $attr_price[$key];
                           $att_data['attr_stock'] = $attr_stock[$key];
                           $att_data['attr_jifen'] = $attr_jifen[$key];
                           $att_data['attr_golds'] = $attr_golds[$key];
                           $att_data['sell_num']   = $sell_num[$key];
                           $att_data['attr_pic']   = $attr_pic[$key];
                           $item_id                     = $id[$key];   
                           if($key>=0 && empty($id[$key])){       
                             M('item_attr')->save($att_data);
                           }else{
                             M('item_attr')->update($att_data,"id=$item_id");                          
                           }                           
                      }
                  }
                   // 保存最小值的属性信息           
                  $ky = array_search(min($attr_price), $attr_price);
       
                  if($ky){
                    $data_min['price']  = $attr_price[$ky];
                    $data_min['points'] = $attr_jifen[$ky];
                    $data_min['stock']  = $allStock;
                    M('item')->update($data_min,"id=$itemId");      
                  }
                  $re_msg['success'] = 1;
                  $re_msg['msg'] = '保存成功！';
              }else{
          ...
          public function save(){       
              ...
              if(!empty($return)){                  
                  ...
                  foreach($attr_name as $vala){
                      if(!empty($vala)){
                           $allStock = $allStock + $attr_stock[$i];
                           $att_data['attr_name']  = $vala;
                  ...
                  // 保存最小值的属性信息
                  $key = array_search(min($attr_price), $attr_price);
                  if($key){
                    $data_min['price']  = $attr_price[$key];
                    $data_min['points'] = $attr_jifen[$key];
                    $data_min['stock']  = $allStock;
                    M('item')->update($data_min,"id=$return");
                  }
                  $re_msg['success'] = 1;
                  $re_msg['msg'] = '保存成功！';
              }else{
          ...
          public function delete()
          {
              ...
              M('item')->delete(array('id'=>$id,'account_id'=>$this->account_id));
